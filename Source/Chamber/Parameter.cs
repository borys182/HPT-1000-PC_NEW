...
namespace HPT1000.Source.Chamber
{
    ...
    public class Parameter
    {
        ...
        {
            bool aRes = false;
            //SPrawdz czy roznica wartosi aktualnej i ostatniej mieci sie w zadanej
            if (firstAsk || (valuePtr != null && Math.Abs(valuePtr.Value_ - valuePtr.LastValueDB) >= diffValue))
            {
                aRes = true;
        ...
        public bool IsTimeSavePara()
        {
            ...
            if (Math.Abs(DateTime.Now.Ticks/TimeSpan.TicksPerMillisecond - lastDateSaved) >= frequency * 1000)
            {
                aRes = true;
